...
*Logistic Regression Model of TikTok Engagement by Danceability, Energy, and Liveness*
```{r}
spotify_df |> 
  mutate(
    tiktok_engagement = `2024_tik_tok_posts` + `2024_tik_tok_likes` + `2024_tik_tok_views`,
    engagement_category = case_when(
      tiktok_engagement < 750e6 ~ "Low",  # Less than 750 million
      tiktok_engagement <= 20e9 ~ "Medium",  # Between 750 million and 20 billion
      tiktok_engagement > 20e9 ~ "High"  # Above 20 billion
    )
  ) |> 
  mutate(engagement_category = factor(engagement_category, levels = c("Low", "Medium", "High")))

logistic_model = glm(
  engagement_category ~ danceability + energy + liveness,
...
model_table = tidy_model |> 
  kable(
    col.names = c("Predictor", "Estimate", "Standard Error", "z-Statistic", "P-Value"),
    caption = "Logistic Regression: Danceability, Energy, and Liveness on TikTok Engagement Categories",
    digits = 3,
    format = "html"
  ) |> 
  kable_styling(
...
Alternatively, logistic regression was used to model TikTok engagement by danceability, energy, and liveness. TikTok engagement was operationalized by combining view, post, and like counts and categorizing into low (<750 million total), medium (>750 million and <20 billion total), and high (>20 billion) engagement. Only liveness was shown to be statistically significant with a p-value of 0.028 and supports the previous finding that songs with fewer live elements are more popular on TikTok. The lack of significance of these predictors in the logistic regression model and the exponentiated estimates being very close to 1 show that music characteristics don't necessarily influence popularity on TikTok.
